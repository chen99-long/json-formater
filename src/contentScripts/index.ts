...
import { buildDom } from './lib/buildDom.ts'
import type { JsonArray, JsonObject, JsonValue } from './lib/types.ts'
...
const cssPromise = new Promise<string>((resolve) => {
  Browser.storage.local.get('themeOverride').then((result) => {
  ...
  const originalPreElement = (() => {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (child.tagName === 'PRE')
        return child as HTMLPreElement
    }
    return null
  })()
